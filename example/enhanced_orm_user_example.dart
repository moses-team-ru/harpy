...
class EnhancedOrmUserExample extends Model with ActiveRecord {
  @override
...
class Post extends Model with ActiveRecord {
  ...
  // Relationships
  Future<EnhancedOrmUserExample?> get author =>
      belongsTo<EnhancedOrmUserExample>('user_id');
...
class Profile extends Model with ActiveRecord {
  ...
  // Relationships
  Future<EnhancedOrmUserExample?> get user =>
      belongsTo<EnhancedOrmUserExample>('user_id');
}
...
class Role extends Model with ActiveRecord {
  ...
  // Relationships
  Future<List<EnhancedOrmUserExample>> get users =>
      belongsToMany<EnhancedOrmUserExample>(
        'user_roles',
        'role_id',
        'user_id',
      );
...
Future<void> _demonstrateBasicOperations() async {
  ...
  final user = EnhancedOrmUserExample()
    ..name = 'John Doe'
  ...
  final invalidUser = EnhancedOrmUserExample()
    ..name = ''
    ..email = 'invalid-email';
  ...
  // Create valid user
  final jane = EnhancedOrmUserExample()
    ..name = 'Jane Smith'
...
Future<void> _demonstrateCopyWith() async {
  ...
  // Find existing user
  final original = await ActiveRecord.find<EnhancedOrmUserExample>(1);
  if (original == null) return;

  print('👤 Original user: ${original.name}, Age: ${original.age}');

  // Create copy with modifications
  final copy = original.copyWith<EnhancedOrmUserExample>(
...
Future<void> _demonstrateStaticMethods() async {
  ...
  // Find by ID
  final user = await ActiveRecord.find<EnhancedOrmUserExample>(1);
  print('🔎 Found user by ID: ${user?.name}');

  // Find all users
  final allUsers = await ActiveRecord.fetchAll<EnhancedOrmUserExample>(
    orderBy: 'created_at ASC',
  );
  print('👥 All users count: ${allUsers.length}');

  // Find with conditions
  final youngUsers = await ActiveRecord.fetchAll<EnhancedOrmUserExample>(
    where: 'age < ?',
    parameters: [30],
    orderBy: 'age DESC',
  );
  ...
  // Count users
  final userCount = await ActiveRecord.count<EnhancedOrmUserExample>();
  print('🧮 Total user count: $userCount');

  // Check if users exist
  final hasAdults = await ActiveRecord.any<EnhancedOrmUserExample>(
    where: 'age >= ?',
    parameters: [18],
  );
  ...
  final newUser = await ActiveRecord.create<EnhancedOrmUserExample>({
    'name': 'Bob Wilson',
...
Future<void> _demonstrateRelationships() async {
  ...
  // Create some posts
  final user1 = await ActiveRecord.find<EnhancedOrmUserExample>(1);
